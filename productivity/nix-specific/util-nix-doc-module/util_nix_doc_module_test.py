...
from util_nix_doc_module import (
    parse_nix_module,
    generate_readme,
    process_nix_files_in_directory,
)
...
def test_parse_nix_module_valid(mock_logging, mock_nix_module):
    ...
    assert (
        result["dotfiles.superhero.time"]["description"]
        == "this option will unlock the power to stop time"
    )
    ...
    assert (
        result["dotfiles.superhero.time.traveler"]["description"]
        == "this option will make you a timetraveler"
    )
...
def test_generate_readme(mock_nix_module):
    ...
    assert (
        '| `dotfiles.superhero.time` | this option will unlock the power to stop time | str | "why not" |'
        in readme
    )
    assert (
        "| `dotfiles.superhero.time.traveler` | this option will make you a timetraveler | int | 1 |"
        in readme
    )
